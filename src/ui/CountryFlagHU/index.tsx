 ...
 */
const CountryFlagHU = (props: SVGProps<SVGSVGElement>) => {
  const { className, width, height, role, 'aria-label': ariaLabel, ...rest } = props;

  const iconWidth: string | number = width || '1.5rem';
  const iconHeight: string | number = height || '1.5rem';
  const iconClass = getElementClass('CountryFlagHU');

  const maskId = getElementId('CountryFlagHUMask');

  /* a11y 기본설정 */
  const iconRole: AriaRole = role || 'img';
  const titleId = getElementId('CountryFlagHUTitle');
  const titleText = ariaLabel || '헝가리';
  ...
  return (
    <svg className={mergeClassNames(iconClass, className)} viewBox={'0 0 24 16'} width={iconWidth} height={iconHeight} fill={'none'} role={iconRole} aria-labelledby={titleId} {...rest}>
      ...
      <g mask={`url(#${maskId})`}>
        ...
        <path d={'M0.923096 10.4614H23.0769V15.3844H0.923096V10.4614Z'} fill={'#6DA544'} />
      </g>
    </svg>
  );
};

export default CountryFlagHU;
