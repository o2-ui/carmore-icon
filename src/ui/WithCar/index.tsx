 ...
 */
const WithCar = (props: SVGProps<SVGSVGElement>) => {
  const { className, width, height, role, 'aria-label': ariaLabel, ...rest } = props;

  const iconWidth: string | number = width || '1.5rem';
  const iconHeight: string | number = height || '1.5rem';
  const iconClass = getElementClass('WithCar');

  /* a11y 기본설정 */
  const iconRole: AriaRole = role || 'img';
  const titleId = getElementId('WithCarTitle');
  ...
  return (
    ...
    >
      ...
      <path
        fillRule={'evenodd'}
        clipRule={'evenodd'}
        d={'M21.0002 6C20.5584 6 20.2002 5.64183 20.2002 5.2V0.8C20.2002 0.358172 20.5584 0 21.0002 0C21.442 0 21.8002 0.358172 21.8002 0.8V5.2C21.8002 5.64183 21.442 6 21.0002 6Z'}
        fill={'currentColor'}
      />
      <path
        fillRule={'evenodd'}
        clipRule={'evenodd'}
        d={'M18 3.0002C18 2.55837 18.3582 2.2002 18.8 2.2002L23.2 2.2002C23.6418 2.2002 24 2.55837 24 3.0002C24 3.44202 23.6418 3.8002 23.2 3.8002L18.8 3.8002C18.3582 3.8002 18 3.44202 18 3.0002Z'}
        fill={'currentColor'}
      />
    </svg>
  );
